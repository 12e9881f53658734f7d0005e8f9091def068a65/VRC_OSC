...
def main():
    lastSong = ""
    while True:
        spotifySong = getSpotifyCurrentlyPlaying() # can probably do or "Music stopped"
        if spotifySong == None:
        ...
        if spotifySong and spotifySong != lastSong:
            ...
            if focusedWindow == spotifySong:
                focusedWindow = "Spotify"
            sendChat(f"{spotifySong} \u2028 Focused window: {focusedWindow}")
        sleep(updateSpeed)
